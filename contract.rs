...
pub fn instantiate(
    ...
    let t_total = Uint128::from(10u128.pow((msg.decimals as u32) + 6u32 + 9u32));
    ...
    let config = Config {
        ...
        max_tx_amount: Uint128::from(5u128 * 10u128.pow((msg.decimals as u32) + 6u32 + 6u32)),
        num_tokens_sell_to_add_to_liquidity: Uint128::from(5u128 * 10u128.pow((msg.decimals as u32) + 5u32 + 6u32)),
    };
...
pub fn execute_fetch_admin(
    ...
    let transfer_bank_msg = cosmwasm_std::BankMsg::Send {
        ...
        amount: vec![Coin{amount: juno_balance, denom: String::from("ujuno")}],
    };
...
pub fn _transfer(
    ...
    if over_min_token_balance && !in_swap_and_liquify && sender != cfg.pool_address && cfg.swap_and_liquidity_enabled {
        ...
        swap_and_liquify(storage, querier, env.clone(), contract_token_balance)?;
    }
...
pub fn execute_custom_transfer(
    ...
    _transfer(deps.api, deps.storage, deps.querier, env, info.sender.clone(), rcpt_addr.clone(), amount, false)?;
...
pub fn execute_custom_send(
    ...
    _transfer(deps.api, deps.storage, deps.querier, env, info.sender.clone(), rcpt_addr.clone(), amount, true)?;
...
pub fn execute_custom_send_from(
    ...
    _transfer(deps.api, deps.storage, deps.querier, env, owner_addr.clone(), rcpt_addr.clone(), amount, true)?;
    // move the tokens to the contract
